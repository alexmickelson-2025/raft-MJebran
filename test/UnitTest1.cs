...
public class LeaderElectionTests
{
    ...
    // Testing # 9 Duplicate Vote Request Handling
    // [Fact]
    // public void TestDuplicateVoteRequest_SameTermRespondsNo()
    // {
    //     // Arrange
    //     var node = new RaftNode { CurrentTerm = 2, VotedFor = Guid.NewGuid() }; // Use RaftNode
    //     var requestForVote = new RequestForVoteRPC(term: 2, candidateId: Guid.NewGuid()); // A different candidate

    //     // Act
    //     var response = node.HandleRequestForVote(requestForVote);

    //     // Assert
    //     Assert.False(response.VoteGranted); // Ensure vote is not granted
    // }


    // Testing # 10 Election Timer Expiry
    [Fact]
